...
class Channel(Enum):
    ...
    Use ``Channel.MAIN_CURRENT`` etc., or the short code via
    :py:meth:`Channel.from_code`.
    """
    ...
    def coerce(cls, value: Union[Channel, str]) -> Channel:
        ...
        raise TypeError(f"expected Channel or str, got {type(value).__name__}")


# Wire-id reverse lookup, including the alias for GPI bitmap.
WIRE_ID_TO_CHANNEL: dict[int, Channel] = {}
for _ch in Channel:
