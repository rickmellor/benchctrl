...
- **Hardware-free** (default): **1142 tests**, running in ~10 minutes
...
- **Hardware-marked** (`@pytest.mark.hardware`): require real
  instruments. Skip gracefully if hardware is absent. 171 tests across
  Arc + DL3031A + DP2031 + QR10x + SDM4065A, each set skipped when its
  instrument isn't connected. The SDM4065A sets are the newest and have
  not yet been run against the meter — see `TEST_PLAN.md`.
